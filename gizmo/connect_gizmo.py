...
async def connect_to_ufa():
    response = requests.get("http://" + login + ":" + password + "@" + ip_ufa + ":" + port + "/api/usersessions/activeinfo")
    if response.status_code == 200:

        print("Подключение к гизмо ТЦБ успешно\n")
        response = requests.get("http://" + login + ":" + password + "@" + ip_ufa + ":" + port + "@127.0.0.1:80/api/hostcomputers")
        # test
        """
...
async def connect_to_dema():
    response = requests.get("http://" + login + ":" + password + "@" + ip_dema + ":" + port + "/api/usersessions/activeinfo")
    if response.status_code == 200:
